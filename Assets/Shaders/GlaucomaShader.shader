    ...
    SubShader {
        ...
        Pass {
            ...
            fixed4 frag (v2f i) : SV_Target {
                ...
                // Reduce saturation in peripheral vision
                // Keep this effect consistent to maintain soft transition at all severities
                float3 gray = dot(col.rgb, float3(0.299, 0.587, 0.114));
                float saturationLoss = peripheralZone * 0.7; // Constant strength for smooth transition
                col.rgb = lerp(col.rgb, gray, saturationLoss);
                
                // Reduce contrast in peripheral vision
                // Also keep consistent for smooth gradation
                float contrastReduction = peripheralZone * 0.5;
                float3 flatGray = gray * 0.6 + 0.2; // Flatten towards mid-gray
                col.rgb = lerp(col.rgb, flatGray, contrastReduction);
